...
def process_doi(doi):
    ...
    if not jrc_authors:
        if original:
            ...
            if hasattr(result, 'matched_count') and result.modified_count:
                LOGGER.warning(f"DOI {doi} updated to remove JRC authors")
                DL.add_doi_process(doi, action='assign_authors',
    ...
    if not ARG.WRITE:
        ...
        try:
            result = DB['dis']['dois'].update_one({'doi': doi}, payload)
            # Record the process only when the write changed something. Logging
            # before the update meant a re-run over an unchanged DOI still added
            # an event, and the default selection re-picks the same recent DOIs
            # every run until they get a newsletter date.
            if hasattr(result, 'matched_count') and result.modified_count:
                print(f"DOI {doi} updated with jrc_author")
                DL.add_doi_process(doi, action='assign_authors',
                                   coll=DB['dis']['processing'],
                                   notes='Auto-assign authors' if ARG.AUTO
                                         else 'Assign authors')
        except Exception as err:
            terminate_program(err)
    if not jrc_authors:
