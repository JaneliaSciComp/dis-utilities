...
- Command-line flags:
    ...
    --key K    The acknowledgement to add (must be a search_regex key). If omitted,
               an interactive menu of the search_regex keys is shown to choose from.
...
def select_key():
    ''' Prompt the user to pick an acknowledgement from the search_regex vocabulary
        (used when --key was not supplied). Aborts if cancelled or nothing is chosen.
        Keyword arguments:
          None
        Returns:
          The selected key (str)
    '''
    try:
...
def initialize_program():
    ''' Connect to the database, resolve the curator, prompt for the acknowledgement
        key if it was not supplied, load the suporg lookup, and validate the key.
        ...
        Returns:
          None
    '''
    global CURATOR, SUPORG, DISCONFIG   # pylint: disable=global-statement
    ...
    if not ARG.KEY:
        ARG.KEY = select_key()    # interactive menu when --key was omitted
    try:
        SUPORG = DL.get_supervisory_orgs(coll=DB['dis'].suporg)
    except Exception as err:
        terminate_program(err)
    # The acknowledgement is restricted to the search_regex vocabulary.
    try:
        if not DB['dis'].search_regex.find_one({"key": ARG.KEY}, {"_id": 1}):
            terminate_program(f"Acknowledgement '{ARG.KEY}' is not a key in search_regex - "
                              "aborting. (Add it to search_regex first.)")
    except Exception as err:
        terminate_program(err)
...
def new_tag():
    ''' Build the tag object for ARG.KEY, resolving code/type from the suporg lookup
    ...
    '''
    code = SUPORG.get(ARG.KEY)
    return {"name": ARG.KEY, "code": code,
            "type": "suporg" if code else "acknowledgement",
            "curator": CURATOR, "updated": datetime.now()}


def merge_tag(existing):
    ''' Apply the merge rules for ARG.KEY against a DOI's current jrc_acknowledge list.
        Keyword arguments:
          existing: current jrc_acknowledge list (list of dicts), may be empty/None
        Returns:
          (new_list, outcome, entry) - outcome is 'added' | 'upgraded' | 'skipped';
          new_list is the value to $set (None when 'skipped'); entry is the single
          tag object that was added or upgraded (None when 'skipped')
    '''
    tags = list(existing or [])
    matches = [tag for tag in tags if tag.get('name') == ARG.KEY]
    if any(tag.get('curator') != IRIS for tag in matches):
        return None, 'skipped', None     # already human-curated - leave untouched
    if matches:                          # only IRIS match(es) - upgrade in place
        changed = None
        for tag in tags:
            if tag.get('name') == ARG.KEY and tag.get('curator') == IRIS:
                tag['curator'] = CURATOR
                tag['updated'] = datetime.now()
                changed = tag
        return tags, 'upgraded', changed
    entry = new_tag()                    # no match - append
    tags.append(entry)
    return tags, 'added', entry


def process(dois):
    ''' Tag each DOI, writing to the DB in --write mode.
        Keyword arguments:
          dois: list of DOI strings
        Returns:
          (results, errors) - results is a list of {doi, collection, outcome};
          errors is a list of {doi, error} for DOIs in neither collection
    '''
    results, errors = [], []
    for doi in dois:
        ...
        if not rec:
            ...
            continue
        new_list, outcome, entry = merge_tag(rec.get('jrc_acknowledge', []))
        COUNT[outcome] += 1
        results.append({"doi": doi, "collection": coll, "outcome": outcome})
        LOGGER.debug(f"{doi} [{coll}] -> {outcome}")
        if ARG.DEBUG and entry is not None:
            print(f"{doi} [{coll}] {outcome}:")
            print(json.dumps(entry, indent=2, default=str))
        if outcome in ('added', 'upgraded') and ARG.WRITE:
            try:
...
def generate_email(results, errors):
    ...
    body = JE.section_header(f"&#127991; Acknowledgement \"{ARG.KEY}\" &mdash; "
                             f"{len(changed):,} DOI(s) updated")
    if changed:
        entries = [(r['doi'], f"{r['collection']} &middot; {r['outcome']}") for r in changed]
        body += JE.doi_card(f"Curator: {CURATOR}", entries, 'good',
                            second_header='Collection &middot; action')
    ...
    if skipped:
        body += JE.body_row(
            ...
            + JE.doi_card("Skipped", [(r['doi'], r['collection']) for r in skipped],
                          'neutral', second_header='Collection'))
    if errors:
        body += JE.body_row(
    ...
    subject = f"Acknowledgement tag \"{ARG.KEY}\" applied"
    if ARG.TEST:
...
def processing():
    ...
    LOGGER.info(f"Tagging {len(dois):,} DOI(s) with acknowledgement '{ARG.KEY}' "
                f"({'WRITE' if ARG.WRITE else 'dry run'})")
    results, errors = process(dois)
    rows = [("DOIs processed", len(results) + len(errors)),
            ("  added", COUNT['added']),
            ("  upgraded (IRIS -> curator)", COUNT['upgraded']),
            ("  skipped (already curated)", COUNT['skipped']),
            ("  not found", COUNT['notfound'])]
    if ARG.WRITE:
        rows.append(("  written", COUNT['written']))
    ...
    if results or errors:
        with open('tag_acknowledgement.json', 'w', encoding='utf-8') as handle:
            json.dump({"key": ARG.KEY, "curator": CURATOR,
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--key', dest='KEY', default=None,
                        help='Acknowledgement to add (must be a search_regex key); '
                             'if omitted, choose from an interactive menu')
