...
def mark_no_acknowledgements(internal):
    ...
    try:
        to_mark = [row['doi'] for row in DB['dis'].dois.find(query, {"doi": 1})]
    except Exception as err:
        terminate_program(err)
    COUNT['no_ack_marked'] = len(to_mark)
    if ARG.WRITE and to_mark:
        try:
            DB['dis'].dois.update_many({"doi": {"$in": to_mark}},
...
def processing():
    ...
    flush_acks(internal)
    mark_no_acknowledgements(internal)
    if not ARG.WRITE:
    ...
    if ARG.SOURCE is None:
        ...
        print(f"No-ack DOIs {verb}: {COUNT['no_ack_marked']:,}")
    if error:
